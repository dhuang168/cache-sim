...
from agentsim.core.des.config import ModelConfig
...
# ─── Contract bridge helpers ───

def make_cache_key(model_id: str, session_id: str, prefix_hash: str) -> "CacheKey":
    """Create a CacheKey from session context. Imports lazily to avoid circular deps."""
    from agentsim.core.contracts import CacheKey
    return CacheKey(model_id=model_id, tokenizer_id="default", prefix_hash=prefix_hash)


def cache_object_to_contract(
    obj: CacheObject, object_id: str, cache_key: "CacheKey",
    worker_id: int = 0, gpu_id: int = 0,
) -> "contracts.CacheObject":
    """Convert internal mutable CacheObject to frozen contract CacheObject."""
    from agentsim.core.contracts import CacheObject as ContractCacheObject
    return ContractCacheObject(
        object_id=object_id,
        cache_key=cache_key,
        size_bytes=obj.size_bytes,
        token_count=obj.token_range[1] - obj.token_range[0] + 1 if obj.token_range else 0,
        created_at_us=obj.created_at_us,
        last_access_us=obj.last_accessed_at_us,
        tier=obj.tier.name if isinstance(obj.tier, Tier) else str(obj.tier),
        worker_id=worker_id,
        gpu_id=gpu_id,
    )
